...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Sexcore methods. For example, calling `JSON.stringify` on a Sexcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
git pull --rebase sexcoin-project master
```
